...
import {getSingleEvents } from '@/utils/api/api';
...
// console.log("single Event------>",event);
  ...
  const date = formatDate(event?.data?.createdAt);
  const remoteImage =event?.data?.image;
  const articleHtml = event?.data?.description ?? '';
  ...
  return (
    <div className="max-w-screen-xl mx-auto p-5 font-nunito">
      <header>
        <div className="relative w-full h-[400px] md:h-[500px] rounded-lg overflow-hidden mt-20">
